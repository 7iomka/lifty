...
if(isset($_POST['form_task'])){ $task = trim($_POST['form_task']);} else{ $task = 'main';}

$admin_emails = Array("info@seosintez.ru");
...
switch($task){
  ...
  $user_email = trim($_POST['user_site--grant']); //сайт
  ...
  // turn on for bots with disabled javascript to prevent spambot actions
    ...
    $subj = "Сообщение с сайта story.tvorim.biz. ".$task_name;
    $mes = "";
    $mes .= "<html>
                      <head>
                        <title>Сообщение с сайта story.tvorim.biz ".$task_name."</title>
                      </head>
                    <body>";
    $mes .= "<table>";
    $mes .= "<tr><th colspan='2'>Данные клиента</th></tr>";
